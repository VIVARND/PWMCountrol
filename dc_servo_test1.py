...
dc_motor_in1_pin = 22  # DC 모터 제어 핀
...
GPIO.setup(dc_motor_in1_pin, GPIO.OUT)
GPIO.output(dc_motor_in1_pin, GPIO.LOW)  # DC 모터를 A방향으로 설정

GPIO.setup(servo_pwm_pin, GPIO.IN)  # PWM 핀을 입력으로 설정
GPIO.setup(servo_pin, GPIO.OUT)

dc_motor_pwm = GPIO.PWM(dc_motor_pwm_pin, 100)  # DC 모터 PWM 설정
...
def control_dc_motor(speed):
    if speed == 0:
        dc_motor_pwm.ChangeDutyCycle(0)  # 모터 OFF
        print("PWM1 - DC 모터 OFF")
    else:
        dc_motor_pwm.ChangeDutyCycle(speed)  # 속도값 사용
